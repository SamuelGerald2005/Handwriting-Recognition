...
# GoogLeNet model for 64x64 grayscale images
class HCCRGoogLeNet64(nn.Module):
    def __init__(self, num_classes=156):  # change to match your dataset
        super().__init__()

        self.pre_layers = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1),  # Modified from 7x7 to 3x3
        ...
        self.maxpool1 = nn.MaxPool2d(2, stride=2)  # 16x16 → 8x8
        ...
        self.maxpool2 = nn.MaxPool2d(2, stride=2)  # 8x8 → 4x4
...
def train_model():
    ...
    model = HCCRGoogLeNet64(num_classes).to(device)
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), "besthcrnet_model.pth")
        else:
            epochs_no_improve += 1
    ...
    model.load_state_dict(torch.load("besthcrnet_model.pth"))
